...
func (server *ServerRedis) subscribe() (chan *Message, error) {
	...
	go func() {
		...
		defer pubsub.Close()
		defer pubsub.Unsubscribe(redisChannelName)
...
func (server *ServerRedis) numSubscribed() (int, error) {
	r, err := server.client.PubSubNumSub(redisChannelName).Result()
	if err != nil {
		return -1, err
	}
	return int(r[redisChannelName]), nil
}
